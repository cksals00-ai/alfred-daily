...
SEEDS = {
    ...
    # 2026-08-21 보강 — '지진'은 GUARD 에 이미 있는데 씨앗에 없었다. 걸리면 인정하지만
    # 찾으러 가지는 않는 비대칭이었고, 그래서 8/21 페루 강진 6.7 이 후보에도 못 올랐다.
    "국제·안보": ["트럼프", "이란", "중국 외교", "일본 총리", "우크라이나", "지진"],
    # 2026-08-21 보강 — 원문은 미 증시·제약을 자주 싣는데 씨앗이 국내 지수에 쏠려 있었다.
    "경제·시장": ["코스피", "환율", "국제유가", "실적 발표", "반도체", "과징금",
                "뉴욕증시", "임상시험"],
    "부동산":    ["아파트 매매", "부동산 세제", "전세"],
    # 2026-08-21 보강 — 씨앗이 구속·화재·사망 셋뿐이라 실종·성범죄가 통째로 안 잡혔다.
    "사건·사고": ["구속", "화재", "사고 사망", "실종", "성추행"],
    ...
    # 2026-08-13 보강. 이 두 칸은 슬롯을 줘도 후보가 안 잡혀 세 번(8/10·8/12·8/13) 비었다.
    # 아래 씨앗은 그날 원문이 실은 사건을 실제로 후보에 올리는지 하나씩 확인하고 넣었다.
    #   연예계 논란 → 배우 하영 친일 의혹 · BTS 뷔 청력 이상
    #   축구협회    → 대한축구협회 심판 성 접대 의혹
    #   실업률      → 7월 청년 실업률 5년 반 만의 최대폭
    #   국세청 세무조사 → 초고액 체납자 특별전담팀 신설
...
GUARD = {
    ...
    "국제·안보": r"(트럼프|국방부|사관학교|합참|주한미군|한미연합|미국|美|중국|中|일본|日|러시아|러|우크라|이란|이스라엘|하마스|북한|北|유럽|EU|나토|정상회담|외교|지진|강진|총리|대통령)",
    ...
    "날씨·기후": r"(폭염|녹조|조류경보|열대야|무더위|호우|장맛?비|장마|태풍(?!상사)|한파|가뭄|기상청|기온|소나기|미세먼지|폭우|폭설|온열질환|고수온|산불|이상기후|기후변화)",
    "사회·생활": r"(취업|일자리|실업|국세청|체납|세무|탈세|연금|출산|저출생|고령|의료|응급|병원|의대|학교|교육|입시|복지|육아|주 ?4일|노동|임금|건강보험)",
    "문화·스포츠": r"(영화|아이돌|K팝|보이그룹|걸그룹|팬덤|박스오피스|관객|드라마|배우|가수|앨범|공연|전시|축제|영화제|야구|축구|배구|농구|골프|리그|대표팀|올림픽|우승|메달|감독)",
}
...
# 2026-08-13: 모든 기증자에 바닥 1 을 준다. 어떤 칸도 0 으로 깎지 않는다.
#   8/13 은 기준 정본(8/12)이 하루밖에 안 됐고 국제·안보 41% 라 탄력 배분이
#   정당하게 발동했는데, 사회·생활과 문화·스포츠를 0 으로 깎았다. 그날 원문은
#   그 두 칸에 5꼭지(축구협회·하영·BTS 뷔·가양동 화재·대청호 녹조)를 실었다.
#   슬롯이 0 이면 적중 확률도 0 이다 — 이건 통계가 아니라 산수다.
#   ANCHOR_MAX_AGE 는 「오래된 기준」을 막지만 「멀쩡한 기준의 과교정」은 못 막는다.
#   같은 기사 풀 A/B: 현행 24%/25% · 배분 끔 29%/31% · 바닥 1 29%/29%.
#   다만 같은 코드로 10분 간격 재크롤 시 12%↔24% 로 흔들렸다. 이 5%p 차이는
#   잡음보다 작다 — 숫자가 근거가 아니라 위 산수가 근거다.
FLOOR = {c: 1 for c in DONORS}
