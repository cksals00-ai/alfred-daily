...
def keys(s):
    ...
    있으므로 어간만 남겨 비교한다 — 뒤에서 한 글자(≥3자)·두 글자(≥4자)를 떼어
    같이 넣는다. 일반 어휘 겹침은 신호가 아니므로 MATCH_STOP 으로 걸러낸다.

    ※ 2026-08-10 재기준. 두 글자 절단 문턱이 5자였을 때 「점검하기」에서 「점검」이
    나오지 않아, 원문 「…추진 상황을 점검하기 위해 2차 민관 합동 회의」와 재현본
    「李대통령, 3대 메가프로젝트 2차 점검」이 겹치는 핵심어 0개로 떨어졌다.
    같은 사건인데 누락 + 오탐으로 두 번 세어진 것이다. 문턱을 4자로 낮추자
    정본 4일 누적이 재현율 35→40% · 정확도 41→46% 로 올라갔다.
    **수집기가 좋아진 게 아니라 자가 고쳐진 것이다** — 8/10 이전 숫자와
    이후 숫자를 같은 축에 놓고 읽으면 안 된다.
    """
    k = set()
    for w in toks(s):
        ...
        if len(w) >= 3:
            k.add(w[:-1])
        if len(w) >= 4:            # 2026-08-10: 5자였는데 4자로 낮췄다.
            k.add(w[:-2])          # 「점검하기」(4자)에서 「점검」이 안 나와
                                   # 재현본이 실제로 잡은 꼭지가 누락으로 세어졌다.
